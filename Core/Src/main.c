...
//HAL_UART_Transmit(&huart2, receiveData, 2, 100);             //发送数据给板子(轮询模式)堵塞
...
if (receiveData[0] == 'R'){
	...
	HAL_GPIO_WritePin(LED_GREEN_GPIO_Port, LED_GREEN_Pin, state);     //未被中断优化的代码
}else if(receiveData[0] == 'B'){
	HAL_GPIO_WritePin(LED_BLUE_GPIO_Port, LED_BLUE_Pin, state);
}*/
//HAL_UART_Transmit(&huart2, (uint8_t*)message, strlen(message), 100);     //发送数据到电脑
//HAL_Delay(1000);
    /* USER CODE END WHILE */
